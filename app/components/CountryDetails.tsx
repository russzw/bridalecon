...
const CountryDetails: React.FC<CountryDetailsProps> = ({ country }) => {
  return (
    <div className="p-6 rounded-lg bg-gray-800 shadow-lg h-full">
      {country ? (
        <div>
          <h2 className="text-3xl font-bold mb-4 text-yellow-400 font-serif underline">{country.country}</h2>
          ...
          <p className="mt-6 text-gray-300 leading-relaxed font-sans">{country.description}</p>
        </div>
      ) : (
        <div className="text-center">
          <h2 className="text-2xl font-bold mb-2 font-serif underline">Country Details</h2>
          <p className="text-gray-400 font-sans">Click on a country to see more details.</p>
        </div>
      )}
    </div>
  );
